...
def worker_import_task(uri, files, queue):
    ...
    try:
        ...
        except ConfigurationError: db = client['test']
        ...
        for idx, file_path in enumerate(files):
            ...
            queue.put(('progress', f"Importing {filename}...", int((idx/total_files)*100)))
                ...
                if ext == '.json':
                    with open(file_path, 'r', encoding='utf-8') as f:
                        try:
                            ...
                            for line in f:
                                if line.strip():
                                    batch.append(json.loads(line, object_hook=json_util.object_hook))
                                    if len(batch) >= 1000:
        ...
        queue.put(('finished', f"Import job finished. Successfully imported {success_count}/{total_files} files."))
    except Exception as e:
        queue.put(('error', f"Critical Import Error: {str(e)}"))
    finally:
        if client: client.close()

# --- EXPORT WORKER (Updated) ---
def worker_export_task(uri, folder, fmt, include_meta, queue, target_colls=None):
    """
    Export logic.
    target_colls: List of strings. If None, exports all non-system collections.
    """
    client = None
    try:
        client = MongoClient(uri)
        ...
        if target_colls:
            ...
            filtered_colls = [c for c in all_colls if not c.startswith('system.') and not c.endswith(('metadata', 'chunks', 'files'))]
        ...
        if fmt == 'sql':
            ...
            with open(full_path, 'w', encoding='utf-8') as f:
                ...
                for idx, name in enumerate(filtered_colls):
                    queue.put(('progress', f"SQL Export: {name}", int((idx/total)*100)))
                    try:
                        sample_docs = list(db[name].find({}).limit(50))
                        if not sample_docs: continue

                        columns = {}
                        if include_meta: columns['_id'] = "TEXT PRIMARY KEY"
                        ...
                        f.write(f'INSERT INTO "{name}" ({", ".join(['"' + c + '"' for c in columns.keys()])}) VALUES\n')
                        ...
                        for i, doc in enumerate(cursor):
                            ...
                            for col in columns.keys():
                                ...
                                if col == '_id' and val is not None: val = str(val)
            ...
            for idx, name in enumerate(filtered_colls):
                ...
                try:
                    ...
                    if fmt == 'json':
                        ...
                        headers = set()
                        for d in sample: headers.update(d.keys())
                        ...
                        with open(path, 'w', newline='', encoding='utf-8') as f:
                            ...
                            for doc in cursor:
                                doc = filter_doc(doc, include_meta)
                                row = {k: (json_util.dumps(v) if isinstance(v, (dict, list, ObjectId)) else v) for k, v in doc.items()}
                                writer.writerow(row)
                except Exception as e:
            ...
            try:
                ...
                for doc in samples:
                    for key, val in doc.items():
                        ...
                        if isinstance(val, ObjectId): type_name = 'ObjectId'
