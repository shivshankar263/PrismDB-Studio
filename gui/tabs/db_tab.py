...
class ConnectWorker(QThread):
    finished = Signal(object, object) # client, db
    error = Signal(str)
    ...
    def run(self):
        try:
            client, db, err = DBManager.connect(self.uri)
            if err:
                self.error.emit(err)
            else:
                self.finished.emit(client, db)
...
class DatabaseTab(QWidget):
    ...
    def init_ui(self):
        ...
        current_dir = os.path.dirname(os.path.abspath(__file__))
        icons_dir = os.path.abspath(os.path.join(current_dir, "../../assets/icons"))
        ...
        # Fix 1: Prevent vertical expansion
        self.progress.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Fixed)
        # Fix 2: Force fixed height to match QSS (15px) prevents layout jumps
    ...
    def connect_mongo(self, uri):
        ...
        self.conn_bar.conn_btn.setEnabled(False)
        self.conn_bar.uri_input.setEnabled(False)
        self.log_view.append(f"Connecting to {uri}...")
        
        # Start Worker
        self.connect_worker = ConnectWorker(uri)
        self.connect_worker.finished.connect(self.on_connect_success)
        self.connect_worker.error.connect(self.on_connect_error)
        self.connect_worker.start()
        
        return True # Return True to indicate process started (UI handles state)

    def on_connect_success(self, client, db):
        self.client = client
        self.db = db
        ...
        # Update UI via ConnectionBar
        self.conn_bar.set_connected_state()
    ...
    def on_connect_error(self, error):
        self.log_view.append(f"Connection Error: {error}")
        QMessageBox.critical(self, "Connection Error", error)
        
        # Reset UI
        self.conn_bar.set_disconnected_state()
        self.connect_worker = None
